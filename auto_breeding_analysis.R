...
find_latest_breeding_dir <- function(base_path) {
  ...
  if (length(breeding_candidates) == 0) {
    # 如果直接子目录下没有，则递归搜索所有名为 breeding 的目录
...
main <- function() {
  ...
  # 自动选择基因型文件（HapMap 格式通常为 .hmp.txt 或 .txt）
  ...
  # 导出 GAPIT_FUN 变量，并且每个 worker 需要 source GAPIT 函数
